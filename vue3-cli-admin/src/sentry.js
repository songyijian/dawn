...
import routes, { asyncRouterMap } from '@/router/routes'
...
function routerFilter(r, rmap) {
  return r.filter((item) => {
    ...
    if (rmap.has(name)) {
      ...
      item.meta.btns = btns // new Set(btns)
      return true
    }
    return false
  })
...
GET_meInfo().then(({ code, data }) => {
  if (!code) {
    const { permissions, roles, user, perms } = data

    // let rolesRouter = asyncRouterMap
    ...
    const routerTree = rolesRouter.concat(routes)
    store.commit('SET_ROUTERS', routerTree)
