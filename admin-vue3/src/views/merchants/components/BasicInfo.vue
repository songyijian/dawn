...
<template>
  <div v-if="config.merchant">
    ...
    <MerchantsRole :inData="config.merchant" :disabled="true" :env="config.env"></MerchantsRole>
    <PersonalInformation :inData="config.merchant" :disabled="config.disabled" :env="config.env"></PersonalInformation>

    <h3>Account information</h3>
    <OutletsInformation :inData="config.merchant" :disabled="config.disabled" :env="config.env"></OutletsInformation>
    <BusinessDocuments :inData="config.merchant" :disabled="true" :env="config.env"></BusinessDocuments>

    <h3>Review Status</h3>
    <Approval :inData="config.current_approval" ref="approvalForm" :disabled="true" :env="config.env"></Approval>

    <div v-if="approvalStatus === 301">
      <p class="redtip">Please contact the merchant to confirm account cancellation application.</p>
    ...
    <div class="btn-box">
      ...
      <template v-if="approvalStatus === 1 || approvalStatus === 201 || approvalStatus === 203 || approvalStatus === 202 || approvalStatus === 301 || approvalStatus === 302">
        <!-- 1、201、203、202、301、302 冻结 -->
        <el-button type="primary" @click="befSbu('Freeze')">冻结 Freeze</el-button>
      </template>

      <template v-if="approvalStatus === 400">
        <!-- 400 Unfreeze -->
        <el-button type="primary" @click="befSbu('Unfreeze')">Unfreeze account</el-button>
      </template>

      <template v-if="approvalStatus === 301">
        <!-- 301 退驻  退驻待审核-->
        <el-button type="primary" @click="befSbu('DeletionUnder')">Deletion rejected</el-button>
        <el-button type="primary" @click="befSbu('DeletionUnder')">Deletion under review</el-button>
      </template>

      <template v-if="approvalStatus === 103 || approvalStatus === 1 || approvalStatus === 203 || approvalStatus === 302">
        <!-- 103、1、203、302.入驻驳回 -->
        <el-button type="primary" @click="edit" v-if="config.env !== 'edit'">编辑 Edit</el-button>
        <el-button type="primary" v-if="config.env == 'edit'" @click="subEdit">Submit Edit</el-button>
      </template>

      <template v-if="approvalStatus === 103 || approvalStatus === 1 || approvalStatus === 203 || approvalStatus === 302">
        <!-- 303.入驻驳回 -->

        <el-button type="primary" @click="edit" v-if="config.env !== 'edit'">通过退驻 Edit</el-button>
        <!-- <el-button type="primary" v-if="config.env == 'edit'" @click="subEdit">Submit Edit</el-button> -->
      </template>
  ...
  <el-dialog v-model="config.dialogVisible" title="" width="70%">
    ...
    <el-form ref="dialogForm" :model="dialogData" label-width="140px">
      <template v-if="dialogData.status == 'Return' || dialogData.status == 'Reject'">
      ...
      <template v-if="dialogData.status == 'Freeze'">
        <el-form-item label="Freeze reason" prop="remark" :rules="verify('required', 'string')">
          <el-input v-model="dialogData.remark" type="textarea"></el-input>
        </el-form-item>
        <el-form-item label="Freeze time" prop="freeze_time" :rules="verify('required', 'int')">
          <el-row :gutter="10">
            <el-col :span="20">
              <el-input v-model.number="dialogData.freeze_time" type="number"></el-input>
            </el-col>
      ...
      <template v-if="dialogData.status == 'Unfreeze'">
        <FreezeInfo v-if="approvalStatus === 400" :inData="config.current_approval"></FreezeInfo>
      </template>
    </el-form>
...
// const test = () => {
//   config.merchant.id_number = '00000000000000000000'
// }

const query = async type => {
  const { code, data, message } = await POST_merchantApprovalInfo({ merchant_id })
  if (!code) {
...
const subEdit = async () => {
  ...
  if (approvalStatus === 103) {
    ...
    const { merchant_id, email, user_address, store_address, business_time, lng, lat } = config.merchant
    req = { merchant_id, email, user_address, store_address, business_time, lng, lat }
  }
  const { code, message } = await ajax(req)
  if (!code) {
