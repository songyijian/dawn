...
<template>
  ...
  >
    ...
    <el-form-item label="gender" prop="gender">
      <el-select
        v-model.number="config.gender"
        :rules="verify('required', 'int')"
      >
        <el-option
          v-for="(key, val) in GENDER"
          :key="val"
          :value="Number(val)"
          :label="key"
        ></el-option>
    ...
    >
      <el-select v-model.number="config.id_type">
        <el-option
          v-for="(key, val) in MERCHANT_ID_TYPE"
          :key="val"
          :value="Number(val)"
          :label="key"
        ></el-option>
    ...
    <el-form-item
      label="Referral code"
      prop="recommend_mobile"
      :rules="verify('required', 'string')"
    >
      <el-input
        v-model.number="config.recommend_mobile"
        type="string"
      ></el-input>
    </el-form-item>
    ...
    <el-form-item
      ...
      prop="id_pics"
      :rules="verify('required', 'arryRepeatedString')"
    >
      <FileUpload
        @uploaderCallback="uploaderCallback($event, config.id_pics)"
        :disabled="disabled"
      >
        ...
        <div class="img-box">
          <p v-for="(url, index) in config.id_pics" :key="url">
            <el-image
              ...
              fit="cover"
              :src="url"
              :preview-src-list="config.id_pics"
            ></el-image>
            <el-button type="text" @click="rmImage(index, config.id_pics)"
              >Remov</el-button
            >
          </p>
        </div>
