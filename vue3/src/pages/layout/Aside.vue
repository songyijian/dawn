...
<template>
  ...
  <el-menu default-active="2" class="el-menu-vertical-demo" router :collapse="false" @open="handleOpen" @close="handleClose">
    <el-sub-menu v-for="item in routers" :index="item.path" :key="item.path">
      ...
      <el-menu-item-group v-if="item.children">
        <el-menu-item v-for="citem in item.children" :key="citem.path" :index="item.path + '/' + citem.path">{{ citem.meta.title }}</el-menu-item>
      </el-menu-item-group>
    </el-sub-menu>
  </el-menu>
</template>
...
console.log(routers, asyncRouterMap)
